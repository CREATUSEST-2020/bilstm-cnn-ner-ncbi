...
class NCBINerProcessor(object):
    """Processor for the NCBI Disease NER data set """
    ...
    def _read_file(cls, input_file):
        ...
        for s in sentences:
            ...
            final = (" ").join(word_list)+"\t"+(" ").join(label_list)
            s_list.append(final)
        return s_list
...
def load_word_matrix(args, word_vocab):
    ...
    with open(os.path.join(args.embed_dir, args.embed_file), 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
...
def load_examples(args, mode):
    processor = NCBINerProcessor(args)

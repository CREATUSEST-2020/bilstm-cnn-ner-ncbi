...
class BiLSTM_CNN(nn.Module):
    ...
    def forward(self, word_ids, char_ids, mask, label_ids):
        ...
        if label_ids is not None:
            batch_size, max_len = output.size(0), output.size(1)
            lstm_feats = output.view(batch_size * max_len, -1)
            tags = label_ids.view(-1)
            loss = nn.functional.cross_entropy(lstm_feats, tags)
            #loss = nn.CrossEntropyLoss(lstm_feats, tags, reduction="mean")
